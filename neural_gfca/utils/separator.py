...
def separate_batch(args: Namespace, unk_args: list[str], initialize: Callable, separate: Callable):
    ...
    for src_filename in pbar(src_fname_list):
        ...
        separate(src_filename, dst_filename.with_suffix(f".{args.dst_ext}"), ctx, args, unk_args)
...
def main(add_common_args: Callable[[ArgumentParser], None], initialize: Callable, separate: Callable):
    ...
    add_common_args(sub_parser)
    sub_parser.add_argument("--src_ext", type=str, default="wav")
    sub_parser.add_argument("--dst_ext", type=str, default="wav")
