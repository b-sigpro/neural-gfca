...
def make_dataset(args, unk_args):
    ...
    mix_path = Path.cwd() / args.mode / "notsofar1-sim" / "derev"
    act_path = Path.cwd() / args.mode / "notsofar1-sim" / "act"
    src_path = Path.cwd() / args.mode / "notsofar1-sim" / "src"
    ...
    hdf_name = Path("./hdf5") / f"supervised.notsofar1-sim-nsrc{args.n_src}-{args.mode}.hdf5"
        ...
        for widx, wav_filename in enumerate(pbar(wav_filename_list[rank::size])):
            ...
            try:
                ...
                for sidx in range(n_src):
                    wav, _ = sf.read(src_path / (wav_filename.stem + f"-s{sidx}.wav"), dtype="float32")
                    wav_src.append(wav)
                wav_src = np.stack(wav_src, axis=-1)  # [T, M, S]
            ...
            for grp_name_, duration_, n_mic_, n_src_ in zip(
                ...
                g.create_dataset("src", [n_mic_, n_src_, duration_], "float32")
            ...
            if grp_name is not None:
                ...
                f[f"{grp_name}/src"][:] = wav_src.transpose(1, 2, 0)
...
def submit_jobs(args, unk_args):
    ...
    for mode in ["train", "dev"]:
        fname_job = job_path / f"{mode}.sh"
        fname_stdout = out_path / f"{mode}.out"
